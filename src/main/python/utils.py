...
def find_cars(img, ystart, ystop, cells_per_step,
              scale, model, orient, color_space,
    ...
    for xb in range(nxsteps):
        for yb in range(nysteps):
            ...
            if test_prediction == 1:
                ...
                start_coord = (xbox_left, ytop_draw + ystart)
                end_coord = (xbox_left + win_draw, ytop_draw + win_draw + ystart)
                car_locations.append((start_coord, end_coord))

    return car_locations


def merge_detected_windows(img, box_list, threshold):
